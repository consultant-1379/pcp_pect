...
#define LIST_TO_MEDIA_STR(NAME, ...) #NAME,
     ...
     * It must be at least 10 (for 0.1 s), recommended is 1000 for 1.0 ms.
 ...
 */
	 ...
	 * new flow, it must be initialized to zero. The size of this data is given by the function
	 * ipoque_pace_get_sizeof_flow_data. (See ipoque_pace_get_sizeof_flow_data) It can be set to NULL for non-TCP/UDP traffic.
...
#define IPOQUE_DETECTION_FASTPATH_NOT_USED 0xFFFFFFFF
	...
	enum ipoque_http_request_method_enum {
        ...
        IPOQUE_HTTP_REQUEST_METHOD_RPC_IN = 17,
        IPOQUE_HTTP_REQUEST_METHOD_RPC_OUT = 18
	};
	...
	/**
	 * This function returns the HTTP status code, the complete status code line and its length of a HTTP response packet.
	 * @param ipoque_struct the detection module, not NULL
	...
	/**
	 * This function returns the pointer to the HTTP Content-Length request or response header field and the length of this line.
	 * @param ipoque_struct the detection module, not NULL
	...
	enum ipoque_http_processing_state {
		IPOQUE_HTTP_PROCESSING_NOT_AVAILABLE = 0,
		IPOQUE_HTTP_PROCESSING_ONGOING = 1,
		IPOQUE_HTTP_PROCESSING_DONE = 2
	};

	/**
	 * This function returns 1 if the current HTTP transaction has
	 * been processed and no further processing is done (for this
	 * transaction). Value might change again for the next request.
	 *
	 * @param ipoque_struct the detection module, not NULL
	 * @return DONE if processing is done, ONGOING if processing is still active, NOT_AVAILABLE if not HTTP
	 *
	 * works with IPOQUE_PACE_DYNAMIC_UPGRADE
	 */
	enum ipoque_http_processing_state ipoque_detection_http_transaction_handling_done(struct ipoque_detection_module_struct *ipoque_struct);

    /**
     * This function returns the pointer to the content of the HTTP header field X-Forwarded-For (XFF).
     * @param ipoque_struct the detection module, not NULL
     * @param xff_content       double pointer to the content string, pointer will be set by this function,
     *                          NULL if no string is available or fastpath is used
     * @param xff_content_len   pointer to the length of the content string
     *
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
     */
    void ipoque_detection_get_http_x_forwarded_for(struct ipoque_detection_module_struct *ipoque_struct,
                                                   unsigned char **xff_content,
                                                   u16 * xff_content_len);

    /**
     * This function returns the http referer
     * @param ipoque_struct the detection module, not NULL
     * @param referer double pointer to the referer string, pointer will be set by this function, NULL when no referer available
     * @param referer_len length of the referer string
     *
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
     */
    void ipoque_detection_get_http_referer(struct ipoque_detection_module_struct *ipoque_struct,
                                           unsigned char **referer,
                                           u16 * referer_len);

    /**
     * This function returns the pointer to the content of the HTTP header field x-online-host.
     * @param ipoque_struct the detection module, not NULL
     * @param xoh_content       double pointer to the content string, pointer will be set by this function,
     *                          NULL if no string is available or fastpath is used
     * @param xoh_content_len   pointer to the length of the content string
     *
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
     */
    void ipoque_detection_get_http_x_online_host(struct ipoque_detection_module_struct *ipoque_struct,
                                                 unsigned char **xoh_content,
                                                 u16 * xoh_content_len);
...
#define IPOQUE_DETECTION_MAIL_TYPE_TO		1
	 ...
	 * @param tmp_store is a temporary storage, must be initialized with 0 before the first call for each packet. not NULL.
	 */
...
#define IPOQUE_ENABLE_GAMEKIT_SIP_CORRELATION	        1
     ...
     */
    u8 ipoque_pace_set_flow_id_correlation_mode(struct ipoque_detection_module_struct * const ipoque_struct,
                                                const ipoque_flow_id_correlation_activation_mode_t mode,
                                                void *(*ipoque_malloc) (unsigned long size, void *userptr),
                                                void (*ipoque_free) (void *ptr, void *userptr));
#endif
 ...
 */
...
#if defined(IPOQUE_DETECTION_MEASURE_TCP_FLOW_LATENCY) || IPOQUE_PACE_API_VARIANT >= 3
     ...
     * @param flow void pointer for the buffer, the flow is in
...
#if defined(IPOQUE_ENABLE_FLOW_OOO_COUNTER) || IPOQUE_PACE_API_VARIANT >= 3
	/**
...
#define IPOQUE_OPENVPN_SAFE_DETECTION			1
	 ...
	 * @param openvpn_mode the mode can be IPOQUE_OPENVPN_AGGRESSIVE_DETECTION or IPOQUE_OPENVPN_SAFE_DETECTION.
	 *
	 * works with IPOQUE_PACE_DYNAMIC_UPGRADE
	 */
	 ...
	 *
	 * @param ipoque_struct the detection module, not NULL
	 * @param generic_voice_search_bitmask the bitmask. See function description, not NULL.
	 *
	 * works with IPOQUE_PACE_DYNAMIC_UPGRADE
	 */
 ...
 */
...
#if defined(IPOQUE_MEASURE_RTP_PERFORMANCE) || IPOQUE_PACE_API_VARIANT >= 3
	...
	/**
	 * This function enables the global and per flow rtp performance measurement.
     * It must be called directly after initializing the detection module and before
     * setting the detection bitmask or getting the flow memory size.
     *
	 * Additional flow memory is used when enabled.
     * Once enabled it cannot be turned off again.
     *
	 * @param ipoque_struct the detection module, not NULL
	 * @param ipoque_malloc function pointer to memory allocator, not NULL
	 * @param ipoque_free function pointer to memory free function, not NULL
     * @return: 0 on success or 1 incase of an error.
     * 
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
	 */

    u8 ipoque_enable_rtp_performance_measurement( struct ipoque_detection_module_struct * const ipoque_struct,
                                                  void *( *ipoque_malloc ) ( unsigned long size, void *userptr ),
                                                  void ( *ipoque_free ) ( void *ptr, void *userptr ) );
     ...
     *
     * This function is deprecated. ipoque_get_rtp_flow_stats should be used for
     * new implementations instead.
     *
	 * @param ipoque_struct the detection module, not NULL
    ...
    /**
     * Reset internal sequence number state for the given RTP flow.
     * @param ipoque_struct the detection module, not NULL
     * @param flow a pointer to the connection state machine.
     *
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
     */
    void ipoque_reset_rtp_flow_sequence_numbers(struct ipoque_detection_module_struct *ipoque_struct, void *flow);
#endif
    ...
    /**
     * @brief SSL Client Hello status codes
     */
    enum ipoque_ssl_client_hello_status {
        IPOQUE_SSL_CLIENT_HELLO_FAILURE = 0, /*!< Client Hello decoding failed. */
        IPOQUE_SSL_CLIENT_HELLO_SUCCESS      /*!< Client Hello decoding succeed. */
    };

    /**
     * @brief SSL Client Hello information structure
     */
    struct ipoque_ssl_client_hello {
        enum ipoque_ssl_client_hello_status error; /*!< Client Hello status code. */
        const u8 *server_name;                     /*!< Server Name Indication (SNI) pointer. */
        u16 server_name_len;                       /*!< Server Name Indication (SNI) field length. */
        const u8 *session_id;                      /*!< SSL Session ID pointer. */
        u16 session_id_len;                        /*!< SSL Session ID length. */
    };

    /**
     * @brief Retrieve information about the SSL client hello
     *
     * This function can be called for every SSL packet and will return a static
     * ipoque_ssl_client_hello structure. If a specific information is not
     * given in the SSL handshake, all pointers will be set to NULL and all
     * length values will be 0. If the parsing fails, the
     * ipoque_ssl_client_hello_status will give an indication.
     *
     * @param[in]   ipoque_struct   the detection module structure
     * @return the pointer to the ipoque_ssl_client_hello structure
     *
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
     */
    const struct ipoque_ssl_client_hello * ipoque_ssl_decode_client_hello(struct ipoque_detection_module_struct * ipoque_struct);
...
#define IPOQUE_SSL_DECODE_MIDSTREAM	6
		/**
...
#define IPOQUE_ID_DUMP_DATA_SIZE  128
...
#if defined(IPOQUE_ENABLE_CORRELATING_FLOW_ID) || defined(IPOQUE_ENABLE_CORRELATING_SIP_FLOW_ID) || defined (IPOQUE_SUPPORT_FLOW_ID_CORRELATION)
	/**
     ...
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
	 */
    void ipoque_detection_update_flow_id(struct ipoque_detection_module_struct *ipoque_struct,
                                         void *flow, void *src, void *dst, u64 id);
#endif
...
#define IPOQUE_OS_RESULT_AVAILABLE 0x40
#define IPOQUE_NAT_RESULT_CHANGED 0x80
	...
	/**
	 * query the PACE release version. In case of enabled
	 * dynamic upgrade, it's the version of the main library.
	 * To get the version of the currently active library
	 * use ipoque_pace_get_active_version() function.
	 * The PACE version is described by a major and minor
	 ...
	 * version number.
	 *
	 * @param version structure to be filled, if NULL nothing happens
	 */
	 ...
	 */
    enum ipoque_dynamic_load_return_enum ipoque_pace_dynamic_upgrade_load_library(struct ipoque_detection_module_struct *ipoque_struct,
                                                                                  const char *filename,
																				  void *(*ipoque_malloc) (unsigned long
																										  size,
																										  void
	 ...
	 */
    enum ipoque_dynamic_load_return_enum
            ipoque_pace_update_cloned_library(struct ipoque_detection_module_struct *cloned_ipoque_struct,
...
#define IPOQUE_PACE_ONLY_COMPLETE_L4_PAYLOAD ( 1 << 2 )
     ...
     * IP packet is not fragmented or the packet is the first fragment. If the
	...
	/**
     * This function returns either the id of the real protocol
     * (if protocol history feature is enabled)
     * or the id of the unknown protocol (value IPOQUE_PROTOCOL_UNKNOWN).
	 * 
	 * @param ipoque_struct the detection module, not NULL
	 * @returns the id of the real protocol or the id of the unknown protocol.
	 *
	 * works with IPOQUE_PACE_DYNAMIC_UPGRADE
	 */
    ...
    /**
     * this function returns the long name of the given group
     *
     * @param ipoque_struct the detection module, not NULL
     * @param group_id the group ID
     * @return a pointer to a statically allocated null-terminated string
     */
    const char *ipoque_pace_get_group_long_name(struct ipoque_detection_module_struct *ipoque_struct, u16 group_id);
    ...
    /**
     * this function returns the long name of the given application ID
     *
     * @param ipoque_struct the detection module, not NULL
     * @param application_id application ID
     * @return pointer to a statically allocated string
     */
    const char *ipoque_pace_get_application_long_name(struct ipoque_detection_module_struct *ipoque_struct,
                                                      u32 application_id);
    ...
    /**
     * @brief ipoque_pace_get_number_of_supported_applications returns the number of supported applications
     * @param ipoque_struct the detection module, not NULL
     * @return number of supported applications
     *
     * works with IPOQUE_PACE_DYNAMIC_UPGRADE
     */
    u16 ipoque_pace_get_number_of_supported_applications(struct ipoque_detection_module_struct * ipoque_struct);
...
#if defined(IPOQUE_DETECTION_SUPPORT_CLIENT_SERVER_INDICATION) || IPOQUE_PACE_API_VARIANT >= 3
	 ...
	 * internal information in case of the fast path), can be NULL.
	 *
...
#if defined(IPOQUE_ENABLE_LICENSING) || IPOQUE_PACE_API_VARIANT >= 3
	 ...
	 * @param ipoque_struct the detection module, not NULL
     * @param param either IPOQUE_NAT_DETECTION_ENABLED or IPOQUE_NAT_DETECTION_DISABLED
	 * @param ipoque_malloc function pointer to a memory allocator
    ...
    u8 ipoque_pace_set_nat_detection_mode(struct ipoque_detection_module_struct * const ipoque_struct,
                                          const enum ipoque_nat_detection_mode param,
                                          void *(*ipoque_malloc) (unsigned long size, void *userptr),
                                          void (*ipoque_free) (void *ptr, void *userptr));
     ...
     * @param[in] ipoque_malloc     function pointer to a memory allocator
     * @param[in] ipoque_free       function pointer to a memory free function
