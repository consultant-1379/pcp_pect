...
#define IPQ_TOH_TIMEOUT_INFINITE	0xFFFFFFFF
     ...
     */
    struct timeorderedhash *ipoque_to_hash_create(unsigned long memory,
            u32 UserBufferSize,
            u32 UniqueBufferSize,
            IPOQUE_TIMESTAMP_COUNTER_SIZE timeout, void (*timeout_callback)
            (IPOQUE_TIMESTAMP_COUNTER_SIZE,
             u8 *, u8 *, void *),
            void * (*ipq_malloc)(unsigned long), void (*ipq_free)(void *))
     ...
     * @param timeout_callback callback function for a timeout. The parameters are the last access time of the element, a pointer to the key, the data and a global user pointer. Can be NULL
     ...
     * @param toh the pointer to the hash table, not NULL
     * @param unique the pointer to the key, not NULL
     * @param new pointer to an u8. This will be set to != 0 when the insert has created a new entry in the hash table, not NULL
    ...
    /** deprecated hash destroy function for ipoque_to_hash_create, please use ipoque_to_hash_destroy2
     * @see ipoque_to_hash_destroy2
     */
    void ipoque_to_hash_destroy(struct timeorderedhash *, void (*ipq_free)(void *)) __attribute__((deprecated));
    ...
    /** get the reason for the last remove operation. It can be a timeout, a delete or an hash table overflow
     * @param toh The pointer to the hash table, not NULL
